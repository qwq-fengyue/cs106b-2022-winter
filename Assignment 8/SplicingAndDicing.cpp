...
Nucleotide* toStrand(const string& str) {
    ...
    for (char ch: str) {
        ...
        if (head == nullptr) {
            ...
            tail->next = nucleotide;
            nucleotide->prev = tail;
            tail = nucleotide;
        }
    }
    return head;
...
Nucleotide* findFirst(Nucleotide* dna, Nucleotide* target) {
    ...
    int lenDNA = len(dna);
    int lenTarget = len(target);
    if (lenDNA < lenTarget)
        return nullptr;

    for (int i = 0; i <= lenDNA - lenTarget; i++, dna = dna->next) {
        if (dna->value == target->value) {
            Nucleotide* copyDNA = dna;
            Nucleotide* copyTarget = target;
            bool flag = true;
            for (int i = 0; i < lenTarget; i++) {
                if (copyDNA->value != copyTarget->value) {
                    flag = false;
                    break;
                }
                else {
                    copyDNA = copyDNA->next;
...
bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    if (dna == nullptr)
        return true;
    Nucleotide* find = findFirst(dna, target);
    if (!find)
        return false;
    int lenTarget = len(target);
    Nucleotide* prevFind = find->prev;
    for (int i = 0; i < lenTarget; i++) {
        Nucleotide* next = find->next;
        delete find;
        find = next;
    }
    if (prevFind == nullptr && find == nullptr) {
        dna = nullptr;
    }
    else if (prevFind == nullptr) {
        dna = find;
        dna->prev = nullptr;
    }
    else if (find == nullptr) {
        prevFind->next = nullptr;
    }
    else {
        prevFind->next = find;
        find->prev = prevFind;
