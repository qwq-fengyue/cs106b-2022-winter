...
int sumVaule(const Set<Shift>& soFar) {
    int sum = 0;
    for (Shift elem: soFar)
        sum += valueOf(elem);
    return sum;
}

bool shift_allowed(const Set<Shift>& soFar, Shift elem, int maxHours) {
    if (maxHours < lengthOf(elem))
        return false;
    else {
        for (Shift elemPicked: soFar) {
            if (overlapsWith(elemPicked, elem))
                return false;
        }
        return true;
    }
...
Set<Shift> highestValueScheduleForRec(const Set<Shift>& elems, const Set<Shift>& soFar, int maxHours) {
    if (maxHours < 0)
        error("The maxHours is negative!");
    else {
        Set<Shift> result;
        if (elems.isEmpty())
            result = soFar;
        else {
            Shift elem = elems.first();
            if (shift_allowed(soFar, elem, maxHours)) {
                auto elemPicked = highestValueScheduleForRec(elems - elem, soFar + elem, maxHours - lengthOf(elem));
                auto elemRejected = highestValueScheduleForRec(elems - elem, soFar, maxHours);
                int value_picked = sumVaule(elemPicked);
                int value_rejected = sumVaule(elemRejected);
                result = value_picked > value_rejected ? elemPicked : elemRejected;
            }
            else
                result = highestValueScheduleForRec(elems - elem, soFar, maxHours);
        }
        return result;
    }
}
