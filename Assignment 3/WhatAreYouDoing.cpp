...
Set<string> allEmphasesOfRec(const Vector<string>& elems, const Vector<string>& soFar) {
    Set<string> reslut;
    if (elems.isEmpty()) {
        string sentence;
        for (string unit: soFar) {
            sentence += unit;
        }
        reslut += sentence;
    }
        ...
        string elem = elems[0];
        if (isalpha(elem[0])) {
            string upper = toUpperCase(elem);
            string lower = toLowerCase(elem);
            reslut += allEmphasesOfRec(elems.subList(1), soFar + upper);
            reslut += allEmphasesOfRec(elems.subList(1), soFar + lower);
        }
        else {
            reslut += allEmphasesOfRec(elems.subList(1), soFar + elem);
        }
    }
    return reslut;
...
Set<string> allEmphasesOf(const string& sentence) {
    Vector<string> units = tokenize(sentence);
    return allEmphasesOfRec(units, {});
}
