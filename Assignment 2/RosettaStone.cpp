...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    ...
    if (kGramLength <= 0)
        error("The kGramLength parameter is not positive");

    unsigned length = str.length();  //避免for循环中重复调用str.length()
...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    for (string key: input) {
        sum += pow(input[key], 2);
        if (sum == 0)
            error("The input Map does not contain any nonzero values");
    }
