...
DataPoint HeapPQueue::dequeue() {
    ...
    while (i < logicalSize) {
        ...
        if (rightChild <= logicalSize) {
            if (elems[rightChild].weight <= elems[leftChild].weight
                    && elems[rightChild].weight < elems[i].weight) {
                swap(elems[rightChild], elems[i]);
                i = 2 * i + 1;
            }
            else if (elems[leftChild].weight <= elems[rightChild].weight
                     && elems[leftChild].weight < elems[i].weight) {
                swap(elems[leftChild], elems[i]);
                i = 2 * i;
            }
            else
                break;
        ...
        else if (leftChild <= logicalSize) {
            if (elems[leftChild].weight < elems[i].weight) {
                swap(elems[leftChild], elems[i]);
                i = 2 * i;
            }
            else
