...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    ...
    string personOne = possibleLinks.firstKey();
    remaining.remove(personOne);
    for (string personTwo: possibleLinks[personOne]) {
        if (remaining.containsKey(personTwo)) {
            remaining.remove(personOne);
            remaining.remove(personTwo);
            if (hasPerfectMatching(remaining, matching)) {
                Pair partner(personOne, personTwo);
                matching += partner;
                return true;
            }
            else
                remaining.put(personTwo, possibleLinks[personTwo]);
        }
    }
    return false;
}
...
void maximumWeightMatchingRec(const Map<string, Map<string, int>>& possibleLinks,
                              ...
                              Set<Pair>& bestMatching, const Set<Pair>& matching,
    ...
    string personOne = remaining.first();
    for (string personTwo: possibleLinks[personOne]) {
        if (remaining.contains(personTwo)) {
            Pair partner(personOne, personTwo);
            maximumWeightMatchingRec(possibleLinks, remaining - personOne - personTwo,
                                     bestMatching, matching + partner,
                                     maxValue, value + possibleLinks[personOne][personTwo]);
        }
    }
    maximumWeightMatchingRec(possibleLinks, remaining - personOne,
                             bestMatching, matching, maxValue, value);
    return;
}
Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    ...
    int maxValue = 0;
    maximumWeightMatchingRec(possibleLinks, remaining, bestMatching, {}, maxValue, 0);
    return bestMatching;
}
