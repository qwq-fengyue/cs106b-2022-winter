...
bool canBeMadeDisasterReadyRec(const Map<string, Set<string>>& roadNetwork,
    ...
    if (numCities == 0 && !remaining.isEmpty())
        return false;
    ...
    if (canBeMadeDisasterReadyRec(roadNetwork, numCities - 1, supplyLocations,
                                  remaining - city - roadNetwork[city])) {
        supplyLocations.add(city);
        return true;
    }
    for (string neighbor: roadNetwork[city]) {
        if (canBeMadeDisasterReadyRec(roadNetwork, numCities - 1, supplyLocations,
                                      remaining - neighbor - roadNetwork[neighbor])) {
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
    ...
    return canBeMadeDisasterReadyRec(roadNetwork, numCities,
                                     supplyLocations, remaining);
}
