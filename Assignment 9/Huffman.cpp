...
EncodingTreeNode* huffmanTreeFor(const string& str) {
    ...
    for (char ch: chFreq) {
        ...
        pq.enqueue(leafNode, chFreq[ch]);
    }
    while(pq.size() > 1) {
        EncodingTreeNode* treeNode = new EncodingTreeNode;
        int zero = pq.peekPriority();
        EncodingTreeNode* zeroNode = pq.dequeue();
        int one = pq.peekPriority();
        EncodingTreeNode* oneNode = pq.dequeue();
        int freq = zero + one;

        treeNode->zero = zeroNode;
        treeNode->one = oneNode;
        pq.enqueue(treeNode, freq);
    }
    return pq.dequeue();
}
 ...
 */
void makeHuffmanTable(EncodingTreeNode* tree, Map<char,Vector<Bit>>& huffmanTable, Vector<Bit> path) {
    if (tree->zero == nullptr)
        huffmanTable[tree->ch] = path;
    else {
        makeHuffmanTable(tree->zero, huffmanTable, path + 0);
        makeHuffmanTable(tree->one, huffmanTable, path + 1);
    }
}

Queue<Bit> encodeText(const string& str, EncodingTreeNode* tree) {
    Map<char,Vector<Bit>> huffmanTable;
    makeHuffmanTable(tree, huffmanTable, {});
    Queue<Bit> bits;
    for (char ch: str) {
        for (Bit bit: huffmanTable[ch]) {
            bits.enqueue(bit);
        }
    }
    return bits;
