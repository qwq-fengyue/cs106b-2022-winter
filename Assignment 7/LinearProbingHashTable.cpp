...
bool LinearProbingHashTable::insert(const string& elem) {
    ...
    for (int i = 0; i < numSlots; i++) {
        if (elems[hashValue].type == SlotType::EMPTY
                || elems[hashValue].type == SlotType::TOMBSTONE) {
            elems[hashValue].value = elem;
            elems[hashValue].type = SlotType::FILLED;
            numElems++;
            return true;
        }
...
bool LinearProbingHashTable::contains(const string& elem) const {
    int hashValue = HashFuc(elem);
    for (int i = 0; i < numSlots; i++) {
        if (elems[hashValue].value == elem
                && elems[hashValue].type == SlotType::FILLED)
            return true;
        else if (elems[hashValue].type == SlotType::EMPTY)
            return false;
        hashValue = (hashValue + 1) % numSlots;
    }
    return false;
