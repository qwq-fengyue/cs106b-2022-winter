...
bool RobinHoodHashTable::insert(const string& elem) {
    if (contains(elem) || numElems == numSlots)
        return false;

    int hashValue = HashFuc(elem);
    string str = elem;
    for (int i = 0; i < numSlots; i++) {
        if (elems[hashValue].distance == EMPTY_SLOT) {
            elems[hashValue].value = str;
            elems[hashValue].distance = i;
            numElems++;
            return true;
        }
        else if (i > elems[hashValue].distance) {
            swap(elems[hashValue].value, str);
            swap(elems[hashValue].distance, i);
        }
        hashValue = (hashValue + 1) % numSlots;
    }
    return false;
}
...
bool RobinHoodHashTable::contains(const string& elem) const {
    int hashValue = HashFuc(elem);
    for (int i = 0; i < numSlots; i++) {
        if (elems[hashValue].value == elem && elems[hashValue].distance != EMPTY_SLOT)
            return true;
        else if (elems[hashValue].distance == EMPTY_SLOT || i > elems[hashValue].distance)
            return false;
        hashValue = (hashValue + 1) % numSlots;
    }
    return false;
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    for (int i = 0; i < numSlots; i++) {
        if (elems[hashValue].value == elem && elems[hashValue].distance != EMPTY_SLOT) {
            elems[hashValue].distance = EMPTY_SLOT;
            numElems--;
            while (elems[(hashValue + 1) % numSlots].distance > 0) {
                swap(elems[hashValue], elems[(hashValue + 1) % numSlots]);
                elems[hashValue].distance--;
                hashValue = (hashValue + 1) % numSlots;
            }
            return true;
        }
